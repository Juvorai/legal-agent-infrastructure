...
5. **No cover note in the contract file (firm rule).** Do not add a cover note, transmittal letter, explanation page, or any other new front matter to the redlined agreement. Do not insert a cover note as the first page or as a separate section of the contract .docx. If an explanation of edits is needed, write it as a separate memo file (following the ben-writing-style-2 skill) and deliver that memo alongside the redline. Never merge the memo into the contract.
6. **Verification:** After building the .docx, run `scripts/verify_redline.py` to confirm every ORIGINAL text appears as `w:delText` and every PROPOSED text appears as `w:ins`, and that the file contains no cover-note front matter. If verification fails, rebuild.
